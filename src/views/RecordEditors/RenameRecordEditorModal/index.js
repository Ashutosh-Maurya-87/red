...
import { MAX_RE_NAME } from '../CreateRecordEditor/configs';
import { PROCESS_MANAGER_MSG, ERROR_MESSAGES } from '../../../configs/messages';
...
function RenameRecordEditorModal({
  isOpen,
  handleClose,
  handleUpdatedName,
  name,
  ...
  const onChangeName = ({ target: { value } }, i) => {
    const validName = value.substring(0, MAX_RE_NAME);
    setName(validName);
  };
  ...
  const onClickUpdate = async evt => {
    ...
    try {
      ...
      const is_exists = false;

      if (is_exists) {
        showErrorMsg(PROCESS_MANAGER_MSG.process_already_exist);
        setLoading(false);
        return;
      }
  ...
  return (
    <Dialog
      className="customized-rename-record-modal"
      maxWidth="xs"
      fullWidth
      ...
      <form noValidate onSubmit={onClickUpdate}>
        ...
        <DialogContent>
          <TextField
            ...
            autoFocus
            fullWidth
            placeholder="Record Editor Name"
            error={validNameFunc()}
...
RenameRecordEditorModal.propTypes = {
  doneText: string,
...
RenameRecordEditorModal.defaultProps = {
  doneText: 'Update',
  title: 'Rename Record Editor',
};

export default RenameRecordEditorModal;
