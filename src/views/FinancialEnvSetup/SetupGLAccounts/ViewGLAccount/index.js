...
import { TYPES } from '../../../Dimensions/EditDimension/configs';
import {
  GL_ACCOUNT_HEADERS,
...
import { getValidCellValue } from '../helper';

import './styles.scss';
import { fetchDimension } from '../../../../services/Dimensions';

class ViewGLAccount extends React.Component {
  state = {
    isImport: false,
    showLoader: false,
    hasGLAccount: true,
  ...
  fillInitialData = async ({ data, metadata } = {}) => {
    ...
    // Commented - To show only headers when Data is 0
  ...
  handleChanges = async (ele, updatedCells) => {
    try {
      ...
      const columns = this.getFormattedHeaders({ metadata: glAccountsMeta });
      const [firstCell = {}] = updatedCells || [];
      const { col = '', newValue = '' } = firstCell || {};

      // Prevent deselection of any dropdown value.
      if (
        col > -1 &&
        columns[col] &&
        columns[col].type &&
        columns[col].type == GL_HEADER_TYPES.dropdown &&
        newValue == ''
      ) {
        this.spreadsheet.undo();
        return;
   ...
   */
  render() {
    const { isImport, showLoader, hasGLAccount, tableHeight } = this.state;
